...
[![1](images/1.png)](#co_input_output_operations_CO2-1)
...
[![2](images/2.png)](#co_input_output_operations_CO6-2)
...
[![1](images/1.png)](#co_input_output_operations_CO8-1)
...
[![7](images/7.png)](#co_input_output_operations_CO12-8)
...
[![1](images/1.png)](#co_input_output_operations_CO13-1)
...
交换金融数据最广泛使用的格式之一是 `CSV` 格式。尽管它并没有真正标准化，但它可以被任何平台处理，并且绝大多数与数据和金融分析有关的应用程序都可以处理。前一节展示了如何使用标准 Python 功能将数据写入 `CSV` 文件并从 `CSV` 文件中读取数据（参见[“读写文本文件”](#reading_and_writing_text_files)）。`pandas` 使得整个过程更加方便，代码更加简洁，并且总体执行更快（还可以参见[图 9-3](#io_plot_03)）：
...
[![4](images/4.png)](#co_input_output_operations_CO33-4)
...
[![6](images/6.png)](#co_input_output_operations_CO34-6)
...
[![8](images/8.png)](#co_input_output_operations_CO35-8)
...
+   关于`NumPy`的I/O功能概述可在`SciPy`网站上找到：[*http://docs.scipy.org/doc/numpy/reference/routines.io.html*](http://docs.scipy.org/doc/numpy/reference/routines.io.html)。

+   对于使用`pandas`进行I/O，请参阅在线文档中的相应部分：[*http://pandas.pydata.org/pandas-docs/stable/io.html*](http://pandas.pydata.org/pandas-docs/stable/io.html)。
...
^([1](ch09.html#idm140277658125664-marker)) 这里，我们不区分不同级别的 RAM 和处理器缓存。当前内存架构的最佳使用是一个独立的主题。

^([2](ch09.html#idm140277655563600-marker)) 要了解 Python 可用的数据库连接器的概述，请访问[*https://wiki.python.org/moin/DatabaseInterfaces*](https://wiki.python.org/moin/DatabaseInterfaces)。与直接使用关系型数据库不同，对象关系映射器，例如[SQLAlchemy](https://www.sqlalchemy.org/)，通常非常有用。它们引入了一个抽象层，允许更加 Pythonic、面向对象的代码。它们还允许更容易地在后端将一个关系型数据库更换为另一个。

^([3](ch09.html#idm140277655097520-marker)) 请参阅[*https://www.sqlite.org/lang.html*](https://www.sqlite.org/lang.html)以了解 `SQLite3` 语言方言的概述。

^([4](ch09.html#idm140277654035024-marker)) 请参阅[*http://docs.scipy.org/doc/numpy/reference/arrays.datetime.html*](http://docs.scipy.org/doc/numpy/reference/arrays.datetime.html)。

^([5](ch09.html#idm140277650901008-marker)) 许多其他数据库需要服务器-客户端架构。对于交互式数据和金融分析，基于文件的数据库在一般情况下会更加方便，也足够满足大多数目的。
