...
经过前向传播，我们从人工神经网络得到预测值。假设网络的期望输出是*Y*，前向传播的预测值是*Y′*。预测输出与期望输出的差异（*Y*–*Y′*）被转换为损失（或成本）函数*J(w)*，其中*w*表示人工神经网络中的权重。³ 目标是优化损失函数（即使损失尽可能小）在训练集上的表现。
...
在下面的示例中，我们使用了`cross-entropy`损失函数，在 Keras 中被定义为`binary_crossentropy`。我们还将使用 adam 优化器，这是默认选项。最后，因为这是一个分类问题，我们将收集并报告分类准确性作为度量标准。⁷以下是 Python 代码：
...
⁵ 有关优化的更多细节，请参阅[*https://oreil.ly/FSt-8*](https://oreil.ly/FSt-8)。
